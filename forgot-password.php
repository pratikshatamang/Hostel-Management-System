...
$stmt=$mysqli->prepare("SELECT id FROM userregistration WHERE email=? AND contactNo=? ");
				...
				if($rs)
				{
					$hashedPassword=hms_password_hash($newPassword);
					$udate=date('d-m-Y h:i:s', time());
					$up=$mysqli->prepare("UPDATE userregistration SET password=?, passUdateDate=? WHERE id=?");
					$up->bind_param('ssi',$hashedPassword,$udate,$uid);
					$up->execute();
					$up->close();
					$pwdReset=true;
				}
...
{ ?>
					<p><?php echo isset($pwdReset) && $pwdReset ? "Password has been reset successfully. Please login." : ""; ?></p>
					<?php }  ?>
								<form action="" class="mt" method="post">
									...
									<input type="password" placeholder="New Password" name="newpassword" class="form-control mb" required>
